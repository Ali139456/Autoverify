...
export function Header() {
  ...
  return (
    <header className="av-site-header fixed inset-x-0 top-0 z-50 bg-white px-3 py-3 dark:bg-ink-950 sm:px-6">
      <div
        className={`mx-auto flex h-[4.25rem] max-w-6xl items-center justify-between gap-3 rounded-2xl border border-slate-200 bg-white px-3 transition-shadow duration-300 dark:border-white/10 dark:bg-ink-900/75 sm:px-4 ${
          scrolled
            ? "shadow-[0_8px_30px_rgba(15,23,42,0.08)] dark:shadow-[0_10px_40px_rgba(0,0,0,0.55)]"
            : "shadow-[0_2px_16px_rgba(15,23,42,0.05)] dark:shadow-[inset_0_1px_0_rgba(255,255,255,0.06)]"
        }`}
      >
        <Logo height={48} priority />

        <nav className="hidden items-center gap-1 rounded-full border border-slate-200 bg-slate-50 p-1 dark:border-white/10 dark:bg-white/[0.05] md:flex">
          {links.map((l) => {
        ...
        <div className="flex shrink-0 items-center gap-2">
          ...
          <button
            ...
            aria-expanded={open}
            className="flex h-10 w-10 items-center justify-center rounded-xl border border-slate-200 bg-slate-50 text-slate-700 transition hover:bg-slate-100 dark:border-white/10 dark:bg-white/5 dark:text-white dark:hover:bg-white/10 md:hidden"
          >
            {open ? (
      ...
      {open && (
        <div className="mx-auto mt-2 max-w-6xl rounded-2xl border border-slate-200 bg-white p-3 shadow-[0_20px_60px_rgba(15,23,42,0.12)] dark:border-white/10 dark:bg-ink-900/90 dark:shadow-[0_20px_60px_rgba(0,0,0,0.6)] md:hidden">
          <nav className="flex flex-col gap-1">
            {links.map((l) => (
              <Link
                ...
                className="rounded-xl px-4 py-3 text-base font-medium text-slate-700 transition hover:bg-slate-50 hover:text-slate-900 dark:text-slate-200 dark:hover:bg-white/5 dark:hover:text-white"
              >
                {l.label}
              </Link>
            ))}
